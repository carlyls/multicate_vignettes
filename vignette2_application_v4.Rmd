...
Packages like `grf` (used for causal_forest) will support missing values in a specific way. The explanations below are from the grf github reference page.^[https://grf-labs.github.io/grf/REFERENCE.html#missing-values-1]
...
```{r, echo=FALSE, results='asis'}
dummies <- rhc_clean %>% select(starts_with("race_"), starts_with("ninsclas_")) 
knitr::kable(head(dummies, 5))
```
...
```{r, echo=FALSE, results='asis'}
rhc_clean_table <- rhc_clean %>% select(-race, -ninsclas)

vars <- colnames(rhc_clean_table)

vars <- c("treatment", "(fake) center", "scoma1",
          "hrt1", "resp1", "bili1","pafi1", "aps1", "temp1","das2d3pc","sod1","liverhx",
          "race_other","race_white","ninsclas_medicare",
          "ninsclas_medicare_medicaid", "ninsclas_no_insurance", "ninsclas_private",
          "ninsclas_private_medicare")

table <- data.frame(Category = c("Treatment","Studies","Outcome","Covariates",
                                 "","","","","","","","","","","","","","",""),
                    Variables = vars,
                    `Data type` = c("num","factor","int","int","num","num","num",
                                  "int","num","num","int","num","int","int","int",
                                  "int","int","int","int"), 
                    Description = c('[treatment] RHC was administered within 24 hours of admission (Yes = 1, No = 0)',
                                    '[studies] Arbitarily created variable referring to the hospitals/centers that RHC was administered at',
                                    '[outcome] Glasgow Coma Score. It is used to assess a neurological status of a patient, with higher indicating a better prognosis.',
                                    'Heart rate',
                                    'Respiratroy rate',
                                    'Bilirubin in critical illness. High levels are a marker of severity.',
                                    'PaO2/FIO2 ratio (P/F ratio) is a measure of oxygenation in the blood used to assess respiratory failure and the severity of conditions like Acute Respiratory Distress Syndrome (ARDS).',
                                    'Acute physiology component of the APACHE score is a value from 0 to 60 that measures a critically ill patients physiological state based on 12 clinical variables collected within the first 24 hours of ICU admission.',
                                    'Temperature',
                                    'DASI (Duke Activity Status Index) is a 12-question, self-administered questionnaire used to measure a patient functional capacity, especially those with cardiovascular disease.',
                                    'Sodium',
                                    'Cirrhosis, Hepatic Failure',
                                    'Race Other',
                                    'Race White',
                                    'Type of medical insurance - Medicare',
                                    'Type of medical insurance - Medicare and Medicaid',
                                    'Type of medical insurance - No insurance',
                                    'Type of medical insurance - Private',
                                    'Type of medical insurance - Private and Medicare'))
table %>% 
  kbl(caption = "Dictionary of variables used in analysis") %>%
...
For CATE estimation, the package implements two methods: (1) the S-learner, which fits a single model for the outcome, treating the treatment as one of covariates (`"slearner"`), and (2) the causal forest, which recursively partitions covariates to best split based on treatment effect heterogeneity. Further details can be found in Brantner et al.(2024)^[Brantner, C. L., Nguyen, T. Q., Tang, T., Zhao, C., Hong, H., & Stuart, E. A. (2024). Comparison of methods that combine multiple randomized trials to estimate heterogeneous treatment effects. Statistics in medicine, 43(7), 1291-1314. https://onlinelibrary.wiley.com/doi/abs/10.1002/sim.9955] or in the 'Background' section of the website^[https://www.elizabethstuart.org/hte/]
...
2. Boxplot of CATEs stratified by study membership 
...
3. Confidence interval plot shoing 95% confidence intervals for all CATEs, sorted by their estiamted CATEs 
...
4. Best linear projection (available only when `estimation_method = "causalforest"`)
...
5. Interpretation tree for visualizing how covariates drive treatment effect heterogeneity.
